<!-- Shared orchestrator contract. Composed onto every host header by install-agents.sh at setup
     time. Edit here, never in the host files — this is the single source of truth. -->
...
1. Run `orca worktree current --json` and check the current worktree's `isMainWorktree` field.
...
- **Monitor** — drive the run with `/orca-orchestrate`: dispatch ready tasks, watch
  `worker_done`/`escalation`/`question`, sweep pending gates, gate on failures, merge green
  work, release workers. `/orca-status` is the read-only sweep when you need the picture without
  blocking.
...
- Never stop or kill a worker on a timeout, idle, or heartbeat — those mean alive, not done.
  `worker-stop` and `worker-abandon` are user-approved actions only, taken through a gate.
