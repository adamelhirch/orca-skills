...
This agent auto-approves everything opencode would otherwise prompt for (external directories,
repeated identical tool calls) so unattended runs never stall. Use that freedom for the task,
not for anything outside the injected brief.
