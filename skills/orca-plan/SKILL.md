...
- **Worker runtime + model** — which runtime executes each task. The default comes from
  `docs/agents/setup.md` (`## Worker runtime`: `opencode`, `claude-code`, `grok`, `hermes`, or
  `freebuff`) and is
...
- runtime: <opencode|claude-code|grok|hermes|freebuff — default: the setup marker's worker runtime>
